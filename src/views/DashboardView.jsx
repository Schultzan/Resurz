  ...
  return (
    <div style={{ fontFamily: bodyFont, color: theme.text }}>
      ...
      <ChartCard
        ...
        subtitle="Ringdiagram: ledig kapacitet, allokerat inom kap, över kap (per avdelning)."
        compact
      >
        {deptBars.length === 0 ? (
      ...
      >
        <ChartCard title="Fördelning (team)" compact>
          {pieMix.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart margin={{ top: 4, right: 4, bottom: 4, left: 4 }}>
                ...
                <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v) => [`${formatHours(v)} h`, "Timmar"]} />
              </PieChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
          ...
          {personBars.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={Math.max(200, personBars.length * 26)}>
              ...
              >
                ...
                <Bar dataKey="Ledigt" stackId="p" fill={theme.surface3} name="Ledigt" radius={[0, 0, 0, 0]} />
      ...
      <ChartCard title="Timmar per kund" subtitle="Färg = person" compact>
        {custStack.rows.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={Math.max(200, custStack.rows.length * 36)}>
            <BarChart data={custStack.rows} margin={{ left: 4, right: 8, top: 4, bottom: 40 }}>
              ...
              <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(v, n) => [`${formatHours(v)} h`, n]} />
      ...
      <ChartCard title="Timmar per internt projekt" subtitle="Färg = person" compact>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10 }}>
        ...
        <MiniTable title="Per kund" custCols={custCols} />
        <MiniTable title="Per internt projekt" intCols={intCols} />
  ...
  return (
    ...
    >
      {deptBars.map((d) => {
        ...
        const slices = [
          { name: "Ledigt", value: d.Ledigt, color: theme.surface3 },
...
function MiniTable({ title, rows, type, custCols, intCols, driftCols }) {
  return (
    <div
      style={{
        border: `1px solid ${theme.border}`,
            ...
            <tbody>
              {custCols.map((c) => (
                <tr key={c.customer.id}>
                  <td style={mtd}>{c.customer.name}</td>
                  <td style={mtdNum}>{formatHours(c.planerade)}</td>
                  <td style={mtdNum}>{c.budgetTimmar > 0 ? formatHours(c.budgetTimmar) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {intCols ? (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            ...
            <tbody>
              {intCols.map((c) => (
                <tr key={c.project.id}>
                  <td style={mtd}>{c.project.name}</td>
                  <td style={mtdNum}>{formatHours(c.planerade)}</td>
                </tr>
              ))}
            </tbody>
